...
void BtAboutDialog::retranslateSwordTab() {
    ...
    QString content("<h3>");
    content += tr("Sword++ library version %1").arg(
                        swordxx::runtimeVersionStr().c_str());
    content += "</h3><p>";
    content += tr("BibleTime makes use of the Sword++ library, a fork of the "
                  "the SWORD Project. Its purpose is to create open-source "
                  "tools that allow programmers and Bible societies to write "
                  "new Bible software more quickly and easily.");
    content += "</p>";
